...
/** 检测状态 */
export type ChordDetectionStatus = 'idle' | 'listening' | 'correct' | 'wrong' | 'completed';

/** 单个音符的检测结果 */
export interface ChordNoteDetectionResult {
  /** 音符索引 */
  index: number;
  /** 是否正确 */
  isCorrect: boolean;
...
/** useChordInversionDetection Hook 的参数 */
interface UseChordInversionDetectionOptions {
  /** 期望的音符列表（MIDI 音符号数组），按从左到右顺序 */
...
/** useChordInversionDetection Hook 的返回值 */
interface UseChordInversionDetectionReturn {
  /** 当前检测状态 */
  status: ChordDetectionStatus;
  /** 当前待检测的音符索引 */
  currentIndex: number;
  /** 检测结果列表 */
  results: ChordNoteDetectionResult[];
  /** 设备列表 */
 ...
 * 严格按从左到右顺序检测用户弹奏的音符
 */
export function useChordInversionDetection({
  expectedNotes,
  octaveTolerance = 0,
  debounceMs = 100,
  enabled = true,
}: UseChordInversionDetectionOptions): UseChordInversionDetectionReturn {
  // 状态
  const [status, setStatus] = useState<ChordDetectionStatus>('idle');
  const [currentIndex, setCurrentIndex] = useState(0);
  const [results, setResults] = useState<ChordNoteDetectionResult[]>([]);
  ...
  const handleNoteEvent = useCallback(
    (event: MidiNoteEvent) => {
      ...
      // 记录检测结果
      const result: ChordNoteDetectionResult = {
        index: targetIndex,
        isCorrect,
