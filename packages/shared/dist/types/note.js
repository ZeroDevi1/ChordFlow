 ...
 * 五度圈顺序（下行）
 */
export const CIRCLE_OF_FIFTHS_DESCENDING = [
 ...
 * 依据音级拼写和起始八度生成连续上行音符。
 */
function buildAscendingNotes(degreeNames, octave) {
    const notes = [];
 ...
 * 获取指定根音和类型的音阶音符序列
 */
export function getScaleNotes(root, type, octave = 4) {
 ...
 * 获取音符的 MIDI 编号
 */
export function getMidiNumber(name, octave) {
    const noteIndex = {
 ...
 * 从 MIDI 编号获取音符
 */
export function getNoteFromMidi(midiNumber) {
